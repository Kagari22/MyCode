...
// https://pintia.cn/problem-sets/91827364500/exam/problems/type/7?problemSetProblemId=91827365879
// from: cpp.json
#define INF 8e18
...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        P B1(x1, y), B2(x2, y);
        P p1 = lineIntersection(Line<LD>{H2, B1}, Line<LD>{L1, L2});
        P p2 = lineIntersection(Line<LD>{H1, B2}, Line<LD>{L1, L2});
        LD l = min(p1.x, p2.x), r = max(p1.x, p2.x);
        if (r <= L1.x || l >= L2.x) continue;
        l = max(l, L1.x);
        r = min(r, L2.x);
    ...
    if (segs.empty()) {
        cout << fixed << setprecision(2) << (L2.x - L1.x) << "\n";
        return;
    }
    ...
    for (int i = 1; i < segs.size(); i++) {
        auto &last = M.back();
        if (segs[i].first <= last.second) {
            last.second = max(last.second, segs[i].second);
        } else {
            M.push_back(segs[i]);
        }
    }
    ...
    LD best = 0;
    best = max(best, M[0].first - L1.x);
    for (int i = 0; i + 1 < M.size(); i++) {
        best = max(best, M[i + 1].first - M[i].second);
    }
    best = max(best, L2.x - M.back().second);
...
signed main() {
    ...
    while (cin >> H1.x >> H2.x >> H1.y) {
        if (H1.x == 0 && H2.x == 0 && H1.y == 0) {
            break;
        }
        H2.y = H1.y;
        solve();
