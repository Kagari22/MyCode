...
LD MaxGap(vector<LD>& p) {
    LD res = 0;
    sort(p.begin(), p.end());
    for (int i = 0; i + 1 < p.size(); i++) {
        res = max(res, p[i + 1] - p[i]);
    }
    res = max(res, p[0] - p.back() + 2 * PI);
    return res;
}
...
void solve() {
    ...
    int maxd = 0;
    for (int i = 0; i < n; i++) {
        maxd = max(maxd, dist2(PP, p[i]));
    }

    vector<LD> maxp;
    for (int i = 0; i < n; i++) {
        if (dist2(PP, p[i]) == maxd) {
            maxp.push_back(atan2(p[i].y - PP.y, p[i].x - PP.x));
        }
    }

    LD maxt = MaxGap(maxp);

    cout << fixed << setprecision(15) << maxt << '\n';
}
