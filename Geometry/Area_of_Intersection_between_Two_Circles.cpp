...
LD edgeCircleArea(const P &A, const P &B, const P &C) {
    LD r = R;
    bool inA = (length(A) <= r);
    bool inB = (length(B) <= r);

    auto sector = [&](const P &U, const P &V) {
        LD ang = fixAngle(atan2l(V.y, V.x) - atan2l(U.y, U.x));
        return 0.5 * r * r * ang;
    };
    ...
    for (int i = 0; i + 1 < ts.size(); i++) {
        LD tL = ts[i], tR = ts[i + 1];
        P U(A.x + D.x * tL, A.y + D.y * tL);
        P V(A.x + D.x * tR, A.y + D.y * tR);
        P M((U.x + V.x) / 2, (U.y + V.y) / 2);
        if (length(M) <= r) {
            area += 0.5 * cross(U, V);
        } else {
            area += sector(U, V);
...
void solve() {
    ...
    if (d >= r1 + r2) {
        area = 0.0;
    } else if (d <= fabsl(r1 - r2)) {
        LD rmin = min(r1, r2);
        area = PI * rmin * rmin;
    } else {
        LD alpha = acos((d * d + r1 * r1 - r2 * r2) / (2 * d * r1));
        LD beta = acos((d * d + r2 * r2 - r1 * r1) / (2 * d * r2));
        LD s1 = r1 * r1 * alpha;
        LD s2 = r2 * r2 * beta;
        LD tri = r1 * r1 * sin(alpha + beta);
        LD term = 0.5 * sqrtl(max<LD>(0.0, 
            ( -d + r1 + r2 ) *
            (  d + r1 - r2 ) *
            (  d - r1 + r2 ) *
            (  d + r1 + r2 )));
