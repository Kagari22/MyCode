...
template<class T>
bool onSegment(const Point<T>& p, const Point<T>& a, const Point<T>& b) {
    // 共线且在区间内
...
void solve() {
    ...
    while (q--) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            long double X = a.x + (b.x - a.x) * (t.y - a.y) / (b.y - a.y);
            if (X > t.x) {
                cnt++;
            }
        }
