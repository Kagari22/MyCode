...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        while (true) {
            int nj = (j + 1) % n;
            LD s1 = fabsl(cross(points[ni] - points[i], points[j] - points[i]));
            LD s2 = fabsl(cross(points[ni] - points[i], points[nj] - points[i]));
            if (s2 >= s1) {
                j = nj;
            } else {
                break;
        ...
        maxd = max(maxd, dist2(points[i], points[j]));
        maxd = max(maxd, dist2(points[ni], points[j]));
    }

    cout << fixed << setprecision(6) << sqrtl(maxd) << '\n';
}
