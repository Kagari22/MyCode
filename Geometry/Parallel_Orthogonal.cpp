...
/*
    定义一个二维点的模板结构体，支持各种基本运算
    T 可以是 int、long long、double、long double 等数值类型
*/
template<class T>
struct Point {
    T x;  // x 坐标
...
Point<T> lineIntersection(const Line<T> &l1, const Line<T> &l2) {
    // 计算参数 t，使得 l1.a + (l1.b - l1.a) * t = 交点
    T numerator   = cross(l2.b - l2.a, l1.a - l2.a);
    T denominator = cross(l2.b - l2.a, l1.b - l1.a);
    return l1.a + (l1.b - l1.a) * (numerator / denominator);
}
...
using LD = long double;    // 实数类型定义为 long double
using P = Point<LD>;       // 方便使用 Point<long double>

constexpr LD eps = 0;      // 误差阈值（可根据需要修改为 1e-9 等）
...
void solve() {
    ...
    if (parallel(s1, s2)) {
        cout << 2 << '\n';
    } else if (dot(p1 - p0, p3 - p2) == 0) {  // 垂直的话点积为0
        cout << 1 << '\n';
    } else {
        cout << 0 << '\n';
    }
