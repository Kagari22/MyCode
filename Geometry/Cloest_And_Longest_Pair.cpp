...
*/
template<class T>
vector<Point<T>> hp(std::vector<Line<T>> lines) {
...
LD brute(const vector<P> &pts, int l, int r) {
    LD d = INF;
    for (int i = l; i <= r; i++) {
        for (int j = i + 1; j <= r; j++) {
            d = min<LD>(d, dist(pts[i], pts[j]));
        }
    }
    return d;
}

LD strip_closest(vector<P> &pts, LD d) {
    auto mind = d;
    int m = pts.size();
    ...
    for (int i = 0; i < m; i++) {
        for (int j = i + 1; j < m && (pts[j].y - pts[i].y) * (pts[j].y - pts[i].y) < mind; j++) {
            mind = min<LD>(mind, dist(pts[i], pts[j]));
        }
    }
    return mind;
}

LD closest_rec(vector<P> &pts, int l, int r) {
    int n = r - l + 1;
    if (n <= 3) {
    ...
    LD dl = closest_rec(pts, l, mid);
    LD dr = closest_rec(pts, mid + 1, r);
    LD d = min(dl, dr);
    ...
    return min<LD>(d, strip_closest(strip, d));
}

LD closest_pair(vector<P> &pts) {
    sort(pts.begin(), pts.end(), [](const P &a, const P &b){
        return a.x < b.x;
    });
    return sqrtl(closest_rec(pts, 0, (int) pts.size() - 1));
}

LD GetLongest(vector<P> &pts) {
    LD res = 0;
    int n = pts.size();
    for (int i = 0, j = 1; i < n; i++) {
        while (cross(pts[(i + 1) % n] - pts[i], pts[j] - pts[i]) < cross(pts[(i + 1) % n] - pts[i], pts[(j + 1) % n] - pts[i])) {
            j = (j + 1) % n;
        }
        res = max<LD>(res, max(dist(pts[i], pts[j]), dist(pts[(i + 1) % n], pts[j])));
    }
    return sqrtl(res);
}
...
void solve() {
    ...
    LD cloest = closest_pair(pts);
    LD longest = GetLongest(hull);
    
    cout << fixed << setprecision(2) << cloest << ' ' << longest << '\n';
}
