...
int brute(const vector<Point<int>> &pts, int l, int r) {
    int d = INF;
    for (int i = l; i <= r; i++) {
        for (int j = i + 1; j <= r; j++) {
...
int strip_closest(vector<Point<int>> &pts, int d) {
    int mind = d;
    int m = pts.size();
    sort(pts.begin(), pts.end(), [](const Point<int> &a, const Point<int> &b){
        return a.y < b.y;
    });
    for (int i = 0; i < m; i++) {
...
int closest_rec(vector<Point<int>> &pts, int l, int r) {
    int n = r - l + 1;
    if (n <= 3) {
    ...
    int dl = closest_rec(pts, l, mid);
    int dr = closest_rec(pts, mid + 1, r);
    int d = min(dl, dr);

    vector<Point<int>> strip;
    for (int i = l; i <= r; ++i) {
...
int closest_pair(vector<Point<int>> &pts) {
    sort(pts.begin(), pts.end(), [](const Point<int> &a, const Point<int> &b){
        return a.x < b.x;
    });
    return closest_rec(pts, 0, (int) pts.size() - 1);
}
